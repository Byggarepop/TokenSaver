...
/// <summary>
/// Strip-and-collapse minifier for X++ (Dynamics 365 Finance &amp; Operations).
/// X++ shares C-style comment syntax (<c>//</c> and <c>/* */</c>) and uses
/// <c>#</c>-prefixed macro directives (<c>#define</c>, <c>#macrolib</c>, etc.),
/// which are preserved on their own line. Kept separate from
/// <see cref="CppMinifier"/> so X++-specific rules can diverge without
/// affecting C/C++ handling.
/// </summary>
internal static class XppMinifier
{
    public static string StripAndCollapse(string src)
    {
        ...
        while (i < src.Length)
        {
            ...
            // Macro directive — preserve as its own line (handles \ continuation)
